...
for file in $(grep -R --exclude='.github' --exclude='.git' $GITHUB_WORKSPACE -e 'KSU_GIT_VERSION' 2>/dev/null | cut -d':' -f 1 | sort | uniq);do [ "$file" != "$0" ] && sed -E -i 's|(KSU_GIT_VERSION \:=).+|\1 '${C_VERSION}'|g' $file;cat $file | grep  'KSU_GIT_VERSION';done
